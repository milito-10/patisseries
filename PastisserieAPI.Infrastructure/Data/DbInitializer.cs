...
namespace PastisserieAPI.Infrastructure.Data
{
    public static class DbInitializer
    {
        public static void Initialize(ApplicationDbContext context, ILogger logger)
        {
            try
            {
                logger.LogInformation("Verificando base de datos y esquemas...");
                
                // Asegurar que las tablas existen (Script de reparación robusto)
                var sql = GetInitializationSql();
                context.Database.ExecuteSqlRaw(sql);
                
                logger.LogInformation("✅ Estructura de base de datos verificada.");

                // Seed Categorías
                SeedCategorias(context, logger);

                // Seed Admin (Opcional, preferiblemente vía migraciones o setup controlado)
        ...
        private static void SeedCategorias(ApplicationDbContext context, ILogger logger)
        {
            ...
            {
                new() { Nombre = "Pasteles", Descripcion = "Pasteles y tortas para toda ocasión", Activa = true },
                new() { Nombre = "Croissants", Descripcion = "Croissants artesanales recién horneados", Activa = true },
                new() { Nombre = "Galletas", Descripcion = "Galletas caseras y gourmet", Activa = true },
                new() { Nombre = "Panes", Descripcion = "Panes artesanales y especiales", Activa = true },
                new() { Nombre = "Macarons", Descripcion = "Macarons franceses de diferentes sabores", Activa = true },
                new() { Nombre = "Tartas", Descripcion = "Tartas dulces y saladas", Activa = true },
                new() { Nombre = "Cupcakes", Descripcion = "Cupcakes decorados para eventos", Activa = true },
                new() { Nombre = "Postres", Descripcion = "Postres variados y especiales", Activa = true },
                new() { Nombre = "Bebidas", Descripcion = "Bebidas calientes y frías", Activa = true },
                new() { Nombre = "Promociones", Descripcion = "Promociones y combos especiales", Activa = true }
            };
            ...
            foreach (var cat in targetCategories)
            {
                if (!context.Set<CategoriaProducto>().Any(c => c.Nombre == cat.Nombre))
                {
                    ...
                    logger.LogInformation("📦 Agregando categoría: {Categoria}", cat.Nombre);
                }
            }
        }
